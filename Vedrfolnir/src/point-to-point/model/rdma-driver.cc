...
TypeId RdmaDriver::GetTypeId (void)
{
	static TypeId tid = TypeId ("ns3::RdmaDriver")
		...
		.AddTraceSource ("ReceiveUdp", "An udp packet is received.",
				MakeTraceSourceAccessor (&RdmaDriver::m_traceRecvPkt))
		;
	return tid;
}
...
void RdmaDriver::Init(void){
	...
	m_rdma->SetReceiveCb(MakeCallback(&RdmaDriver::ReceiveUdp, this));
}
...
void RdmaDriver::ReceiveUdp(Ptr<Packet> p){
	m_traceRecvPkt(p);
}
