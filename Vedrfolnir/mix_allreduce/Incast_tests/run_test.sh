...
printf "%s\n" "${config_files[@]}" | xargs -P 100 -I{} bash -c '
    file={}
    python ./waf --run "scratch/mix_allreduce mix_allreduce/Incast_tests/config/config_${file}.txt"
    python3 mix_allreduce/Incast_tests/graph.py "mix_allreduce/Incast_tests/data/data_${file}"
'
