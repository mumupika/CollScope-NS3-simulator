...
def rating():
    ...
    if DEMO_CLIENT:
        ...
        ################### Flow contention test ####################

        TEST_result = "FF"
        flow_file = open(f'{file_path}/flow.txt', 'r')
        flow_list = []
        result_list = []

        num_flows = int(flow_file.readline().strip())
        for _i in range(num_flows):
            flow_line = flow_file.readline().strip()
            src_id = int(flow_line.split()[0])
            dst_id = int(flow_line.split()[1])
            flow_time = (int(flow_line.split()[4]) - 2) * 1000

            # only consider flows affecting the critical path
            # j = 0
            # index = bisect.bisect(time_sum, flow_time)
            # fs_f = fs[index-1]
            ...
            # if index < len(fs):
            #     fs_f2 = fs[index]
            #     fs_f2 = int(fs_f2[1:fs_f2.index('S')])
            #     if dst_id != fs_f + 1:
            #         j = 1
            # if j == 1:
            #     continue
            ...
            flow_list.append(f'F_{id_to_ip(src_id) }->{id_to_ip(dst_id)}\n{flow_line.split()[3]}')
        ...
        for i, result in enumerate(result_list):
            if TEST_result == "TP" and result == False:
                TEST_result = "FP"
                break
        ...
        with open(f'mix_allreduce/Flow_contention_tests/data_result/{file_n}.txt', 'a+') as f:
            pass
