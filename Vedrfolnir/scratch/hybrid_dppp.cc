...
#include <ns3/rdma-cc-helper.h>
#include <ns3/hybrid-cc.h>
#include <ns3/hybrid-cc-helper.h>
...
uint32_t auto_agent_threshold = 0;
uint32_t epoch_time = 0;
uint32_t step_detect_times = 1;
uint32_t agent_step_fct;

// for PFC storm
uint32_t pfc_storm_node = 0, pfc_storm_port = 0;
double pfc_storm_start = 0.0, pfc_storm_duration = 0.1;

// Hybrid parallel simulation parameters
uint32_t dp_group_size = 4;
uint32_t num_pp_stages = 4;
uint16_t num_chunks = 1;
bool train_mode = true;
uint8_t schedule_mode = 0; // 0=event-driven, 1=static MegatronLM
uint16_t max_inflight = 100;
uint64_t compute_time_fwd = 0.0;
uint64_t compute_time_bwd = 0.0;
uint64_t chunk_size_fwd = 360000000;
uint64_t chunk_size_bwd = 360000000;

ApplicationContainer hybridApps;
...
map<uint16_t, uint16_t> rank2nodeID;
...
Ptr<RdmaCC> ip_to_app(Ipv4Address ip) {
    int node_id = ip_to_node_id(ip);
    // Try hybrid apps first
    for (uint32_t i = 0; i < hybridApps.GetN(); i++) {
        Ptr<HybridCC> h = DynamicCast<HybridCC>(hybridApps.Get(i));
        if (h && ip_to_node_id(h->GetIP()) == node_id)
            return h->GetRdmaCC();
    }
    // Fallback to legacy apps
    int i = 0;
    for (int agent_i : agent_nodes) {
        if (node_id == agent_i)
            return DynamicCast<RdmaCC>(apps.Get(i));
        i++;
    }
    assert(0);
}

Ptr<HybridCC> ip_to_hybrid(Ipv4Address ip) {
    int node_id = ip_to_node_id(ip);
    for (uint32_t i = 0; i < hybridApps.GetN(); i++) {
        Ptr<HybridCC> h = DynamicCast<HybridCC>(hybridApps.Get(i));
        if (h && ip_to_node_id(h->GetIP()) == node_id)
...
Ptr<RdmaCC> ip_to_hybrid_rdma(Ipv4Address ip) {
...
int main(int argc, char *argv[]) {
...
#endif
    {
        ...
        while (!conf.eof()) {
            ...
            if (key.compare("ENABLE_QCN") == 0) {
            ...
            } else if (key.compare("AUTO_AGENT_THRESHOLD") == 0) {
                conf >> auto_agent_threshold;
                std::cout << "AUTO_AGENT_THRESHOLD\t\t" << auto_agent_threshold << '\n';
            } else if (key.compare("EPOCH_TIME") == 0) {
                conf >> epoch_time;
                std::cout << "EPOCH_TIME\t\t\t" << epoch_time << '\n';
            } else if (key.compare("STEP_DETECT_TIMES") == 0) {
                conf >> step_detect_times;
                std::cout << "STEP_DETECT_TIMES\t\t" << step_detect_times << '\n';
            } else if (key.compare("AGENT_STEP_FCT") == 0) {
                conf >> agent_step_fct;
                std::cout << "AGENT_STEP_FCT\t\t\t" << agent_step_fct << '\n';
                ...
                std::cout << '\n';
            } else if (key.compare("PFC_STORM") == 0) {
                conf >> pfc_storm_node >> pfc_storm_port >> pfc_storm_start >> pfc_storm_duration;
                std::cout << "PFC_STORM\t\t\t" << pfc_storm_node << ' ' << pfc_storm_port << ' ' << pfc_storm_start << ' ' << pfc_storm_duration << '\n';
            } else if (key.compare("DP_GROUP_SIZE") == 0) {
                conf >> dp_group_size;
                std::cout << "DP_GROUP_SIZE\t\t\t" << dp_group_size << '\n';
            } else if (key.compare("NUM_PP_STAGES") == 0) {
                conf >> num_pp_stages;
                std::cout << "NUM_PP_STAGES\t\t\t" << num_pp_stages << '\n';
            } else if (key.compare("NUM_CHUNKS") == 0) {
                conf >> num_chunks;
                std::cout << "NUM_CHUNKS\t\t\t" << num_chunks << '\n';
            } else if (key.compare("TRAIN_MODE") == 0) {
                uint32_t v;
                conf >> v;
                train_mode = (v != 0);
                std::cout << "TRAIN_MODE\t\t\t" << (train_mode ? "Yes" : "No") << '\n';
            } else if (key.compare("SCHEDULE_MODE") == 0) {
                uint32_t v;
                conf >> v;
                schedule_mode = v;
                std::cout << "SCHEDULE_MODE\t\t\t" << (int)schedule_mode << '\n';
            } else if (key.compare("MAX_INFLIGHT") == 0) {
                conf >> max_inflight;
                std::cout << "MAX_INFLIGHT\t\t\t" << max_inflight << '\n';
            } else if (key.compare("COMPUTE_TIME_FWD") == 0) {
                conf >> compute_time_fwd;
                std::cout << "COMPUTE_TIME_FWD\t\t" << compute_time_fwd << '\n';
            } else if (key.compare("COMPUTE_TIME_BWD") == 0) {
                conf >> compute_time_bwd;
                std::cout << "COMPUTE_TIME_BWD\t\t" << compute_time_bwd << '\n';
            } else if (key.compare("CHUNK_SIZE_FWD") == 0) {
                conf >> chunk_size_fwd;
                std::cout << "CHUNK_SIZE_FWD\t\t\t" << chunk_size_fwd << '\n';
            } else if (key.compare("CHUNK_SIZE_BWD") == 0) {
                conf >> chunk_size_bwd;
                std::cout << "CHUNK_SIZE_BWD\t\t\t" << chunk_size_bwd << '\n';
            }
            fflush(stdout);
        }
        conf.close();
...
;
    ...
    //
    maxRtt = maxBdp = 0;
    uint64_t minRTT = 0xffffffffffffffff;
    for (uint32_t i = 0; i < node_num; i++) {
        if (n.Get(i)->GetNodeType() != 0)
            continue;
    ...
    printf("maxRtt=%lu minRTT=%lu maxBdp=%lu\n", maxRtt, minRTT, maxBdp);
    ...
    // ========== Create HybridCC applications ==========
    std::vector<Ipv4Address> allNodeIPs;
    std::vector<uint16_t> allNodePorts;
    uint16_t rank_id = 0;

    HybridCCHelper helper;
    helper.SetAttribute("DPGroupSize", UintegerValue(dp_group_size));
    helper.SetAttribute("NumPPStages", UintegerValue(num_pp_stages));
    helper.SetAttribute("NumChunks", UintegerValue(num_chunks));
    helper.SetAttribute("TrainMode", BooleanValue(train_mode));
    helper.SetAttribute("ScheduleMode", UintegerValue(schedule_mode));
    helper.SetAttribute("MaxInflight", UintegerValue(max_inflight));
    helper.SetAttribute("ComputeTimeFwd", TimeValue(MilliSeconds(compute_time_fwd)));
    helper.SetAttribute("ComputeTimeBwd", TimeValue(MilliSeconds(compute_time_bwd)));
    helper.SetAttribute("ChunkSizeFwd", UintegerValue(chunk_size_fwd));
    helper.SetAttribute("ChunkSizeBwd", UintegerValue(chunk_size_bwd));
    helper.SetAttribute("Window", UintegerValue(maxBdp));
    helper.SetAttribute("BaseRtt", UintegerValue(maxRtt));
    helper.SetAttribute("PriorityGroup", UintegerValue(3));

    for (int i : agent_nodes) {
        rank2nodeID[rank_id] = i;
        allNodeIPs.push_back(serverAddress[i]);
        allNodePorts.push_back(10000);

        // Set per-node attributes
        helper.SetAttribute("RankID", UintegerValue(rank_id));
        helper.SetAttribute("IP", Ipv4AddressValue(serverAddress[i]));
        helper.SetAttribute("Port", UintegerValue(10000));
        hybridApps.Add(helper.Install(n.Get(i)));
        rank_id++;
    }
