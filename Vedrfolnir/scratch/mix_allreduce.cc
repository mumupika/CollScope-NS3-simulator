...
int main(int argc, char *argv[])
{
	...
	for(int i : agent_nodes)
	{
		...
		//rank, ip, port, alg, size, win, baseRtt, pg
		RdmaCCHelper clientHelper(rank_id, serverAddress[i], 10000, 1, 360000000, maxBdp, maxRtt, 3);
		rank_id++;
		Ptr<Application> app = clientHelper.Install(node); 
    ...
    {
		Ptr<RdmaCC> app_i = DynamicCast<RdmaCC>(*i);
		for (ApplicationContainer::Iterator j = apps.Begin (); j != apps.End (); ++j)
		{
			Ptr<RdmaCC> app_j = DynamicCast<RdmaCC>(*j);
		...
		app_i->SetPairRTT(pairRtt);
		app_i->SetAutoRTT(auto_agent_threshold);
    }
